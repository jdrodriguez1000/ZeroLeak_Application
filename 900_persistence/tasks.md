...
- T-28 → **No implementada** (segundo Tracer Bullet `ingest`; en curso — pasos 1-9 del flujo completos; paso 10 (bucle TDD) al 42%, 5/12 casos; suspendida a pedido del humano)
...
- T-36 → **No implementada** (paso 10, bucle TDD de `ingest`: 5/12 casos cerrados — bloque 1 caso 1, bloque 2 casos 2-5; faltan bloque 3 casos 6-9 y bloque 4 casos 10-12)
...
| T-28 | Construir el segundo Tracer Bullet: `zlk ingest <CLIENTE> <ruta>` (registro de archivos en capa bronze del tenant, dedupe por `sha256`) | No implementada | 2026-07-13 | alcance acordado con el humano (ver D-17): copia inmutable a bronze, hash sha256, entrada en `manifest.json` status `pending`, dedupe habilita Modo Incremental; no parsea contenido ("archivador notarial"); validación agnóstica de formato; allow-list MVP `.csv`/`.xlsx` (`.txt` fuera); entrada por archivo o carpeta con recorrido plano; **pasos 1-9 completos**, **paso 10 (bucle TDD) al 42%** (5/12 casos, código en `app/src/zeroleak/ingest/core.py`, suite 49 passed); suspendida a pedido del humano tras el Caso 5; continúa en T-36 |
...
| T-36 | Paso 10 del flujo `ingest`: bucle TDD sobre los 12 casos de `plan.md`/`state.json` | No implementada | 2026-07-13 | completado bloque 1 (caso 1) y bloque 2 (casos 2-5) — 5/12 casos: Caso 1 CA-08 (tenant inexistente→`TenantNotFoundError`/exit 2), Caso 2 CA-01 (ingesta csv byte-a-byte+entrada pending), Caso 3 CA-02 (forma del manifest, sin ciclo RED/GREEN, tratado como caracterización/regresión: TSK-06 implementada, TSK-07 cancelada_suspendida), Caso 4 CA-09 (archivo vacío→fallo/exit 1), Caso 5 CA-05 (dedupe sha256→SKIP/exit 0, incl. intra-invocación); código en `app/src/zeroleak/ingest/core.py` y `__init__.py`, tests en `app/tests/test_ingest.py`, fixtures nuevas en `app/tests/conftest.py`; suite 49 passed, sin regresiones; **suspendida a pedido del humano**; falta bloque 3 (casos 6-9) y bloque 4 (casos 10-12) |
