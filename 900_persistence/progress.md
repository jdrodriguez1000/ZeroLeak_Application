...
- **Prioridad inmediata:** paso 13 (final, gate `human_test`) del segundo Tracer Bullet **`ingest`** (T-28/T-39). El PR #2 (`feature/ingest`→`main`) ya está abierto y actualizado con el fix del bug de `cli.py` (T-41). Las pruebas manuales de dedupe-por-contenido y carga de `.xlsx` (T-42) ya se ejecutaron con éxito contra `clients/CLIENTE_DEMO/`. El humano debe:
  1. Opcionalmente confirmar los escenarios restantes de la matriz original (archivo válido → exit 0; carpeta completa con un archivo vacío → exit 1; tenant inexistente → exit 2), inspeccionando `clients/CLIENTE_DEMO/data/bronze/` y `manifest.json` tras cada corrida.
  2. Si todo funciona correctamente, **mergear el PR #2** en GitHub (el agente nunca mergea).
  3. Tras el merge: limpieza — borrar `Data_test/` y, opcionalmente, los datos de prueba generados en `clients/CLIENTE_DEMO/data/`.
