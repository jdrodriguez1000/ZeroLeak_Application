...
| TSK-12 | Test: ingerir un archivo con **mismo nombre** pero **contenido distinto** a uno ya en bronze crea una **nueva** entrada; el archivo se almacena como `<stem>__<sha8><suffix>` sin sobrescribir el original, y `file` es ese nombre con sufijo. | Test "colision-nombre-sufijo" en `test_ingest.py` | tdd_tester | implementada | CA-06 |
| TSK-13 | Nombrado en bronze ante colisión de nombre con contenido distinto: detectar mismo nombre + `sha256` distinto y almacenar con sufijo `__<sha8>`; registrar el nombre almacenado en `file`. | Lógica de nombrado en `core.py` | tdd_coder | implementada | CA-06 |
| TSK-14 | Test: una carpeta con `a.csv`, `b.xlsx`, `c.txt` y `sub/d.csv` registra exactamente 2 entradas (`a.csv`, `b.xlsx`); `c.txt`, `sub/` y `sub/d.csv` no aparecen en bronze, ni en manifest, ni en fallos. | Test "carpeta-recorrido-plano" en `test_ingest.py` | tdd_tester | implementada | CA-03 |
| TSK-15 | Despacho por ruta para carpetas: recorrido plano (`iterdir`, sin recursión), ingerir solo extensiones ∈ allow-list, ignorar en silencio subcarpetas y demás archivos (no van a `failed`). | Despacho de carpeta en `core.py` | tdd_coder | implementada | CA-03 |
| TSK-16 | Test: invocación con argumentos mezclados `[archivo.csv, carpeta/]` ingiere el archivo suelto y los `.csv`/`.xlsx` del primer nivel de la carpeta; conteos y entradas reflejan la unión de ambos orígenes. | Test "args-mezclados" en `test_ingest.py` | tdd_tester | implementada (test de caracterización de CA-04; no hubo RED porque el despacho multi-argumento ya estaba cubierto por `_expand_paths` desde el Caso 7/TSK-15, que se aplica por-elemento sobre `paths`) | CA-04 |
| TSK-17 | Despacho multi-argumento: iterar sobre todos los `paths` (archivo o carpeta) acumulando resultados en las colecciones de `IngestResult` (unión de orígenes, sin duplicar el ledger). | Bucle de despacho multi-ruta en `core.py` | tdd_coder | cancelada_suspendida (sin código nuevo; CA-04 ya cubierto por el código del Caso 7, TSK-15/`_expand_paths`, que ya itera y despacha cada elemento de `paths` de forma independiente) | CA-04 |
| TSK-18 | Test: tres `.csv` con contenido equivalente pero delimitador `,`, `;`, `\|` se ingieren los tres (copia byte a byte, `sha256` sobre bytes), produciendo 3 entradas; en ningún caso se parsea el contenido como CSV. | Test "delimitadores-sin-parseo" en `test_ingest.py` | tdd_tester | implementada (test de caracterización de CA-10; no hubo RED porque el core ya opera exclusivamente sobre bytes crudos desde el Caso 2/TSK-05 en adelante — nunca abre/parsea CSV) | CA-10 |
| TSK-19 | Confirmar (mínimo-suficiente) que el core opera solo sobre bytes: `sha256` y copia sin abrir/parsear; asegurar que 3 contenidos distintos generan 3 `sha256` distintos → 3 entradas. | Verificación/ajuste agnóstico-de-formato en `core.py` | tdd_coder | cancelada_suspendida (sin código nuevo; CA-10 ya cubierto por el código del Caso 2 en adelante, TSK-05/`_hash_file`/`ingest_paths` — el core siempre opera sobre `read_bytes()`/`hashlib.sha256`/`shutil.copyfile` y nunca abre ni parsea contenido, verificado sobre core.py línea a línea; mismo criterio que TSK-07/TSK-17) | CA-10 |
| TSK-20 | Test: invocación con rutas válida + vacía + extensión no permitida + inexistente ingiere solo la válida; las 3 inválidas van a `failed` con su motivo respectivo (`"archivo vacío"`, `"extensión fuera de allow-list"`, `"la ruta no existe"`) y `exit_code == 1`. | Test "mezcla-valida-invalida" en `test_ingest.py` | tdd_tester | implementada | CA-07 |
| TSK-21 | Procesamiento parcial por ruta: cada ruta se valida/procesa de forma independiente (una falla no aborta el resto); consolidación de motivos y derivación final de `exit_code` (0 sin fallos, 1 con ≥ 1 fallo). | Procesamiento parcial + consolidación en `core.py` | tdd_coder | implementada | CA-07 |
| TSK-22 | Test: `git check-ignore` marca como ignorada cualquier ruta bajo `clients/<X>/data/bronze/...` y `clients/<X>/data/manifest.json`, y NO marca `client.yaml` ni `input/` (rutas ficticias, sin crear tenant real; C-01). | Test "gitignore-frontera-pii" en `test_ingest.py` | tdd_tester | implementada (caracterización: la regla `.gitignore` `clients/*/data/` ya vigente desde `client_scaffold` satisface CA-11 de inmediato, sin cambio de código; no hubo ciclo rojo-verde) | CA-11 |
| TSK-23 | Test: el comportamiento completo es observable llamando directamente a `ingest_paths(...)`; y `zlk ingest <CLIENTE> <ruta>...` produce el mismo efecto en disco invocando el core y traduciendo `exit_code` (0 éxito, 1 parcial/fallo, 2 tenant inexistente). | Tests "core-observable" y "cli-equivalencia+exit-codes" en `test_cli_ingest.py` | tdd_tester | implementada | CA-12 |
| TSK-24 | Fachada CLI `zlk ingest <CLIENTE> <ruta>...`: parsear subcomando, resolver `clients_root`, invocar `ingest_paths`, imprimir reporte OK/SKIP/FAIL + resumen, traducir `IngestResult.exit_code` (0/1) y `TenantNotFoundError` → 2 (sin lógica de ingesta propia). | Despacho `ingest` en `cli.py` | tdd_coder | implementada | CA-12 |
| TSK-25 | Prueba de integración end-to-end: ejecutar el comando instalado `zlk ingest` por subprocess sobre un `clients_root` temporal (tenant sintético); verificar éxito y estructura en bronze/manifest, exit code 1 en mezcla válida/inválida, exit code 2 en tenant inexistente, y equivalencia con `ingest_paths` en proceso. | `app/tests/integration/test_ingest_e2e.py` | integration_tester | implementada (5 tests por subprocess real: CA-01 éxito+estructura, CA-07 exit 1 parcial, CA-05 dedupe no-op, CA-08 exit 2 sin residuo, CA-12 equivalencia CLI-real<->core; pytest app/tests -q -> 69 passed, 0 failed, sin fallos e2e; no se tocó producción) | CA-01, CA-12 |
| TSK-26 | Refactor de `core.py` manteniendo verde: extraer helpers cohesivos (validación, hash+copia, nombrado, carga/persistencia del ledger), nombrar constantes de motivos y allow-list, docstrings con referencia a CA. | `core.py` refactorizado | tdd_refactor | implementada (parcial: cubre la parte `core` completa — helpers ya extraídos en refactors de Casos 2/4/5/6/7, constantes `REASON_*`/`ALLOWED_EXTENSIONS` agrupadas y ordenadas, docstrings de módulo/`ingest_paths`/helpers actualizados con referencia a CA-01..CA-10 y comportamiento completo, consolidado en el refactor integral del Caso 10; la parte CLI de CA-01 — fachada `zlk ingest`, aún sin código, Caso 12 — queda fuera de este alcance) | CA-01 (calidad) |
| TSK-27 | Refactor de `cli.py` manteniendo verde: separar parseo, despacho, formateo del reporte y traducción de resultado/errores a exit codes en unidades legibles, sin duplicar con la rama `client new`. | `cli.py` refactorizado | tdd_refactor | implementada (revisión de conjunto: el GREEN del Caso 12 ya siguió el patrón exacto de `client new` -- `_parse_ingest_args`/`_dispatch_ingest`/`_print_ingest_report` con responsabilidad única y nombres simétricos a `_parse_client_new_name`/`_dispatch_client_new`; `_clients_root()` ya es el único punto de resolución de `ZEROLEAK_CLIENTS_ROOT`, compartido sin duplicación entre ambas ramas; docstrings de las 3 funciones ya referencian CA-12. Único ajuste aplicado: `_USAGE` solo documentaba `client new` (mensaje de fallback engañoso ahora que existe `ingest`); se amplió a dos líneas cubriendo ambos subcomandos -- cambio puramente cosmético (ningún test verifica el texto exacto de `_USAGE`, solo que stderr no esté vacío en el fallback; exit code de fallback permanece en 1). No se extrajo un helper común adicional entre `client new` e `ingest` más allá de `_clients_root()`: la traducción de excepciones a exit codes difiere en cardinalidad y semántica entre ambas ramas (`ClientNameError`->2/`TenantExistsError`->3 vs `TenantNotFoundError`->2, y `ingest` delega el 0/1 al propio `IngestResult.exit_code`), por lo que forzar una abstracción común habría sido sobre-ingeniería (E4/NC-2). pytest app/tests -q -> 64 passed, 0 regresiones.) | CA-12 (calidad) |
